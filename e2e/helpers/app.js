...
async function navigateToWorkspace() {
    await waitForToBeVisible(element(by.id('onboarding-view')), 10000);
	await element(by.id('join-workspace')).tap();
	await waitForToBeVisible(element(by.id('new-server-view')), 60000);
	await element(by.id('new-server-view-input')).replaceText(data.server);
	await element(by.id('new-server-view-button')).tap();
	await waitForToBeVisible(element(by.id('workspace-view')), 60000);
	await expect(element(by.id('workspace-view'))).toBeVisible();
}

async function navigateToLogin() {
    await waitForToBeVisible(element(by.id('onboarding-view')), 20000);
    await navigateToWorkspace();
    await element(by.id('workspace-view-login')).tap();
    await waitForToBeVisible(element(by.id('login-view')), 2000);
...
async function navigateToRegister() {
    ...
    await waitForToBeVisible(element(by.id('register-view')), 2000);
...
async function login(username, password) {
    ...
    await waitForToBeVisible(element(by.id('rooms-list-view')), 10000);
}

async function logout() {
    await element(by.id('rooms-list-view-sidebar')).tap();
    await waitForToBeVisible(element(by.id('sidebar-view')), 2000);
    await waitForToBeVisible(element(by.id('sidebar-settings')), 2000);
...
async function starMessage(message){
    ...
    await waitForToBeVisible(element(by.label(messageLabel)), 5000);
